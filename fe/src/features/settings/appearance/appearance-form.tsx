...
const appearanceFormSchema = z.object({
  ...
  font: z.enum(fonts, {
    invalid_type_error: 'Định dạng không hợp lệ, hãy chọn phông chữ.',
...
export function AppearanceForm() {
  ...
  // Giá trị này có thể lấy từ CSDL hoặc API của bạn
  const defaultValues: Partial<AppearanceFormValues> = {
    theme: theme as 'light' | 'dark',
    font,
  }
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription className='font-manrope'>Chọn phông chữ bạn muốn sử dụng trong bảng điều khiển.</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className='space-y-1'>
              ...
              <FormDescription>Chọn giao diện cho bảng điều khiển.</FormDescription>
              <FormMessage />
              <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className='grid max-w-md grid-cols-2 gap-8 pt-2'>
                <FormItem>
                  <FormLabel className='[&:has([data-state=checked])>div]:border-primary'>
